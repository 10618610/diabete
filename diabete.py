...
#titulo
st.write("Prevendo Diabete")
df = pd.read_csv("diabetes.csv")
#cabeçalho

st.subheader("Informações dos dados")
user_input = st.sidebar.text_input("Digite o nome")
...
#dicionário para receber informações
    user_data = {'Gravidez': pregnancies,'Glicose': glicose,'Pressão Sanguínea': blood_pressure,
             'Espessura da pele': skin_thickness,'Insulina': insulin, 'Índice de massa corporal': bni,  'Histórico familiar de diabetes': dpf,
                 'Idade': age}
...
st.subheader("Escolha um arquivo tipo txt")
data = st.file_uploader('Escolha o dataset (.txt)', type = 'txt')
if data is not None:
    df = np.loadtxt(data)
media = df.mean()
forma = df.shape
st.write("media",media)
st.write("shape",forma)